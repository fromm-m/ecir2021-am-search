...
class Dumani(RankingMethod):
    ...
    def __init__(
        ...
        """
        self.sort_col = rf"P(\pi_j|q)_{method}"
        self.cluster_col = f"premiseClusterID_{method}"
        df = load_assignments_with_numeric_relevance()
        df["premise_text_length"] = df["premise_text"].apply(len)
    ...
    def rank(self, claim_id: int, premise_ids: Sequence[str], k: int) -> Sequence[str]:  # noqa: D102
        ...
        for _, cluster in selection.groupby(by=self.cluster_col):
            result.append(cluster.sort_values(by="premise_text_length", ascending=False).first()["premise_id"])
